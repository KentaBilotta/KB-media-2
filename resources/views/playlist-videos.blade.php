        ...
        <div class="d-flex justify-content-center">
            ...
            <div class="d-flex flex-column p-4" style="width: 80vw;">
                <div class="d-flex align-items-center justify-content-between">
                    <div>
                        <h1>{{ $playlist->name }}</h1>
                        <div class="fs-5">{{ count($playlist->videos) }} video</div>
                    </div>
                    <form action="{{ route('playlist-destroy', ['playlist'=>$playlist])}}" method="post">
                        @method('DELETE')
                        @csrf
                        <button class="btn btn-danger btn-delete-me rounded-2 ms-5">Elimina</button>
                    </form>
                </div>
                @if (session('success_remove'))
                    <div class="alert alert-warning mt-3" role="alert">
                        Video rimosso da '{{ session('success_remove')->name }}'
                    </div>
                @endif
                <div class="my-3">
                    @if ($playlist->videos)
                        @foreach ($playlist->videos->reverse() as $video)
                            <a href="{{ route('watch', ['video' => $video]) }}" class="text-decoration-none text-black">
                                <div class="border rounded-4 p-2 my-2 d-flex align-items-center">
                                    ...
                                    <div class="w-75 ms-3">
                                        ...
                                        <div class="mt-3">
                                            <form action="{{ route('remove-from-platlist', ['video' => $video, 'playlist' => $playlist]) }}" method="POST">
                                                @method('DELETE')
                                                @csrf
